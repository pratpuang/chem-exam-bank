...
const SFX=(()=>{let ctx=null,master=null,verb=null,nbuf=null,on=ls.get("cqb_sfx","1")!=="0",vol=Math.max(0,Math.min(100,parseInt(ls.get("cqb_vol","70"),10)||0));
  ...
  const P={
    ...
    bio:d=>{const[a,b]=d>=0?[196,294]:[294,196];pluck(vary(a,.01),.6,{vol:.1});pluck(vary(b,.01),.7,{vol:.085,delay:.09});
      wood(vary(d>=0?392:330,.02),{vol:.05});noise(.24,{type:"lowpass",freq:900,q:.5,vol:.06,attack:.07,wet:.15});},
    ...
    drawer:w=>{noise(.16,{type:"lowpass",freq:500,sweep:4,q:.7,vol:.08,attack:.07,wet:.15});thock({f:vary(150),vol:.09,delay:.14});
      if(w==="pd")glass(vary(1568,.01),{vol:.04,delay:.15,dur:.4});
      else if(w==="td")pluck(vary(392,.01),.4,{vol:.06,delay:.13});
...
const PD=(()=>{
  ...
  function open(){if(TD.isOpen())TD.close();if(!built)build();refreshQ();document.body.classList.add("pdopen");SFX.play("drawer","pd");}
