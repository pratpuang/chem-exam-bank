...
function closeModal(){if(!$("#modal").classList.contains("show"))return;$("#modal").classList.remove("show");INK.close();SFX.play("close");
...
/* ---- poster-pen: ink over the present sheet ----
   Points are stored normalised to #mbody (the question itself, not the viewport or the growing sheet), so a
...
const INK=(()=>{
  ...
  function load(){const t=today();if(S&&S.d===t)return;
    ...
    if(!S||S.d!==t||!S.q||typeof S.q!=="object"){S={d:t,q:{}};ls.del(K);}}
  const save=()=>ls.set(K,JSON.stringify(S));
  const qid=()=>{const q=filtered[mIdx];return q?q.id:"";};
  const list=()=>S.q[qid()]||[];
  function put(a){const id=qid();if(!id)return;if(a.length)S.q[id]=a;else delete S.q[id];}
  function commit(a,before){const h=hist[qid()]=hist[qid()]||[];h.push(before);if(h.length>60)h.shift();put(a);save();frame(3);}
  // layout offsets, not getBoundingClientRect: the sheet's entry animation scales and rotates it
  function size(){if(!shown())return;const w=wrap.offsetWidth,h=wrap.offsetHeight,d=window.devicePixelRatio||1;
    ox=body.offsetLeft;oy=body.offsetTop;bw=body.offsetWidth||1;bh=body.offsetHeight||1;
    [cvH,cvP].forEach(c=>{c.style.width=w+"px";c.style.height=h+"px";c.width=Math.round(w*d);c.height=Math.round(h*d);c.getContext("2d").setTransform(d,0,0,d,0,0);});
    draw(3);}
  // smoothed "ball pen": quadratic curves through the midpoints, straight into the last point
  function path(x,p){const n=p.length,X=i=>ox+p[i]*bw,Y=i=>oy+p[i+1]*bh;x.beginPath();x.moveTo(X(0),Y(0));
  ...
  function draw(m){const st=getComputedStyle(cvP),C=["--ink","--red","--blue","--yel"].map(v=>st.getPropertyValue(v).trim()),
    ...
    if(m&1){xH.clearRect(0,0,cvH.width,cvH.height);xH.save();xH.globalAlpha=.38;
      ...
      a.forEach(s=>{if(s.t==="h"){path(xH,s.p);xH.stroke();}});xH.restore();}
    if(m&2){xP.clearRect(0,0,cvP.width,cvP.height);xP.lineWidth=3.2;xP.lineCap=xP.lineJoin="round";
  ...
  if(window.ResizeObserver){const ro=new ResizeObserver(()=>size());ro.observe(wrap);ro.observe(body);}
  addEventListener("resize",size);
  return{open(){load();setOn(false);size();},close(){cur=null;setOn(false);},
         refresh(){if(!shown())return;cur=null;load();size();},toggle(){setOn(!on);SFX.play("toggle");}};
})();
...
async function saveShot(){const q=filtered[mIdx],b=$("#mshot");if(!q||b.disabled)return;
  ...
  if(shot.key===k&&shot.blob){deliverShot(shot.blob,nm(shot.blob));SFX.play("ok");b.textContent="✅ บันทึกแล้ว";setTimeout(()=>{b.textContent=lbl;},1200);return;}
