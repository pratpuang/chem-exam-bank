...
const INK=(()=>{
  ...
  bar.addEventListener("click",e=>{const b=e.target.closest("button");if(!b||cur)return;b.blur();
    ...
    else if(b.id==="inksplit"){setSplit(!split);SFX.play("toggle");return;}
    else if(b.id==="inkundo"){const h=hist[qid()];if(!h||!h.length)return;const p=h.pop();for(const k in p)put(k,p[k]);save();frame(3);}
    // 🧹 clears what you can see: the question, plus the board while it's on screen (one undo step)
      ...
      if(!p.q.length&&!(p.b&&p.b.length))return;commit(a,p);}
    else if(b.id==="inkwipe"){if(!confirm("ล้างหมึกทั้งหมด? (ทุกข้อ)"))return;S={d:today(),q:{},b:{}};Object.keys(hist).forEach(k=>delete hist[k]);ls.del(K);frame(3);}
    SFX.play("click");});
  ...
  ["pointerup","pointercancel","lostpointercapture"].forEach(ev=>div.addEventListener(ev,e=>{if(e.pointerId!==drag)return;drag=null;ls.set(DK,JSON.stringify(SR));}));
  ...
  return{open(){load();setOn(false);sizeAll();},close(){cur=null;setOn(false);setSplit(false);},
         refresh(){if(!shown())return;cur=null;load();sizeAll();},toggle(){setOn(!on);SFX.play("toggle");}};
})();
