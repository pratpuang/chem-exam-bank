...
/* background scroll lock (SL below pins the body while any overlay is open): no rubber-band on the pinned page, and an
   overlay's own scroller keeps its scroll to itself instead of handing the leftover drag to the page */
html.scrlock{overscroll-behavior:none}
.sheet,#pd,#td,#tm{overscroll-behavior:contain}
#pdscrim{touch-action:none}
...
/* present bottom row: three equal cells, one height (the tallest label sets it), labels centred */
.mctrl{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-top:20px}
.mctrl button{min-height:50px;padding:8px 10px;border:3px solid var(--ink);background:var(--card);font-size:1rem;font-weight:800;line-height:1.25;box-shadow:4px 4px 0 var(--sh);display:flex;flex-wrap:wrap;align-content:center;justify-content:center;text-align:center}   /* each .nw word group is a flex item: lines break only between them */
.mctrl .reveal.flag{border-style:dashed}.nw{white-space:nowrap}
.mfrow{display:flex;align-items:center;gap:10px;margin-top:12px;font-size:.8rem;color:var(--mut)}
.mhint{transition:opacity .6s}.mhint.gone{opacity:0}body.swiped .mhint{display:none}
.mhint .hf{display:none}@media (hover:hover) and (pointer:fine){.mhint .hc{display:none}.mhint .hf{display:inline}}
.sheet.inR{animation:pInR .32s cubic-bezier(.2,1.1,.4,1)}.sheet.inL{animation:pInL .32s cubic-bezier(.2,1.1,.4,1)}
.sheet.outL{animation:pOutL .18s ease-in forwards}.sheet.outR{animation:pOutR .18s ease-in forwards}
.mctrl button:active{transform:translate(4px,4px);box-shadow:none}
.reveal{background:var(--blue)!important;color:#fff}
.solnone{color:var(--mut)}
.mpos{margin-left:auto;font-family:"JetBrains Mono";font-weight:700;color:var(--ink)}
/* poster-pen: two canvases over the sheet's content (highlighter under pen); inert until write mode (✎ beside ✕, or P)
   is on. The controls (✕ ✎ and the bottom button row) sit above them so they stay clickable while writing. The tool bar
...
/* the big tile flips to its electron configuration. The front (.bf) turns inside the fixed-size .pdbig; the back (.pdcfg) is a
   separate card absolutely positioned on the pdinfo grid area (.pdg is its containing block), so it can be wider/taller than the
   front and overlay the Mw table/tiles WITHOUT reflowing anything. The flip animates transform only (compositor): no width/layout
   per frame (the old widening box forced a layout + style recalc every frame, which is what lagged on iPad). The back hinges on the
   front's centre line (origin 1.45u) and turns in from 180deg, so its far edge recedes rather than swinging at the viewer. */
.pdg{position:relative}
.pdbig{flex:none;position:relative;width:calc(var(--u)*2.9);height:calc(var(--u)*2.75);perspective:calc(var(--u)*14);cursor:pointer}
.pdbig.nil{color:var(--mut);cursor:default}
.pdbig>div,.pdcfg{border:4px solid var(--ink);background:var(--card);box-shadow:6px 6px 0 var(--sh);backface-visibility:hidden;-webkit-backface-visibility:hidden;transition:transform .5s cubic-bezier(.2,.7,.2,1)}
.pdbig>div{position:absolute;inset:0}
.pdg.bstill .bf,.pdg.bstill .pdcfg{transition:none}
...
.pdg.bk .bf{transform:rotateY(-180deg)}
.pdcfg{grid-row:1/4;grid-column:3/13;position:absolute;top:0;left:6px;z-index:5;width:max-content;min-width:calc(var(--u)*2.9);max-width:calc(100% - 12px);box-sizing:border-box;
  display:flex;flex-direction:column;cursor:pointer;line-height:1.15;visibility:hidden;transform-origin:calc(var(--u)*1.45) 50%;
  transform:perspective(calc(var(--u)*16)) rotateY(180deg);transition:transform .5s cubic-bezier(.2,.7,.2,1),visibility 0s .5s}
.pdg.bk .pdcfg{visibility:visible;transform:perspective(calc(var(--u)*16)) rotateY(0deg);transition:transform .5s cubic-bezier(.2,.7,.2,1),visibility 0s}
.pdcfg>b{background:var(--ink);color:var(--paper);font:800 calc(var(--u)*.27) "JetBrains Mono",monospace;padding:2px 6px}
.pdcfg>b i{font-style:normal;font-weight:600;opacity:.7;margin-left:.5em}
.pdcfg sup{font-size:.68em;line-height:0}
/* one group per shell n: its subshells, a └─┬─┘ bracket, then the shell's electron count. Groups wrap whole, never split */
.pdcfg .cg{display:flex;flex-wrap:wrap;gap:calc(var(--u)*.1) calc(var(--u)*.14);padding:calc(var(--u)*.1) calc(var(--u)*.12) calc(var(--u)*.06)}
.pdcfg .g{display:flex;flex-direction:column;align-items:stretch;text-align:center;padding:calc(var(--u)*.05) calc(var(--u)*.08) calc(var(--u)*.04)}
.pdcfg .g:nth-child(even){background:rgba(var(--pbi),.08)}
.pdcfg .g>span{font:600 calc(var(--u)*.27) "JetBrains Mono",monospace;white-space:nowrap}
.pdcfg .g>i{position:relative;height:calc(var(--u)*.12);margin:2px calc(var(--u)*.06) calc(var(--u)*.1);border:2px solid currentColor;border-top:0}
.pdcfg .g>i::after{content:"";position:absolute;left:calc(50% - 1px);top:100%;width:2px;height:calc(var(--u)*.1);background:currentColor}
.pdcfg .g>strong{font:800 calc(var(--u)*.42) "JetBrains Mono",monospace;line-height:1;font-variant-numeric:tabular-nums}
.pdcfg p{margin:0;padding:0 calc(var(--u)*.14) calc(var(--u)*.06);font:600 calc(var(--u)*.25) "JetBrains Mono",monospace;color:var(--mut)}
.pdcfg p em{font:700 calc(var(--u)*.25) "Anuphan",sans-serif;font-style:normal;margin-right:.3em}
.pdcfg p span{white-space:nowrap}
.pdcfg small{font:700 calc(var(--u)*.27) "Anuphan",sans-serif;color:var(--mut);padding:0 calc(var(--u)*.14) calc(var(--u)*.08)}.pdcfg small i{font-style:normal;color:var(--red)}
...
@media (prefers-reduced-motion:reduce){#pd,#pdscrim{transition:none}.pde.inq::after,.mem .pde,.mem .pde::before,.pdbig.pdrev{animation:none!important}
  .pdbig>div,.pdcfg{transition-duration:1ms!important}
...
/* ✎ sub-topic drill-down: the ▾ beside a chapter row drops that chapter's exercise sub-topics. Ink bars on a 6mm
   write-box grid, never the yellow exam bars; the label is drawn twice -- ink on the track, paper on the fill (clipped
   to --w) -- so it stays readable wherever the fill ends */
.exhd{display:flex;gap:5px}.exhd .exr,.exhd .sexb{flex:1;min-width:0}
.xtg{flex:none;width:44px;min-height:44px;border:2px solid var(--ink);background:var(--card);color:var(--ink);font:800 1rem "Anuphan",sans-serif;cursor:pointer;padding:0}
.xtg:hover,.xtg:focus-visible{background:var(--yel);color:#141414;outline:0}
.xtg[aria-expanded=true]{background:var(--ink);color:var(--paper)}
.xtg i{display:inline-block;font-style:normal;transition:transform .2s}.xtg[aria-expanded=true] i{transform:rotate(180deg)}
.xbars{display:flex;flex-direction:column;gap:4px;margin-top:4px}.xbars[hidden]{display:none}
.xbar{position:relative;display:block;width:100%;min-height:44px;padding:0;border:2px solid var(--ink);color:var(--ink);font:700 .82rem "Anuphan",sans-serif;text-align:left;cursor:pointer;
  background:linear-gradient(var(--exg) 1px,transparent 1px) 0 0/6mm 6mm,linear-gradient(90deg,var(--exg) 1px,transparent 1px) 0 0/6mm 6mm,var(--card)}
.xl{display:flex;align-items:center;gap:8px;min-height:40px;padding:0 10px}
.xl span{flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.xl b{font:800 .82rem "JetBrains Mono",monospace}
.xf{position:absolute;inset:0;color:var(--paper);clip-path:inset(0 calc(100% - var(--w)) 0 0);animation:xgrow .45s cubic-bezier(.2,.9,.3,1) both;animation-delay:calc(var(--k)*28ms);
  background:linear-gradient(var(--xg) 1px,transparent 1px) 0 0/6mm 6mm,linear-gradient(90deg,var(--xg) 1px,transparent 1px) 0 0/6mm 6mm,var(--ink)}
.xbar:hover,.xbar:focus-visible{outline:3px solid var(--yel);outline-offset:1px}
@keyframes xgrow{from{clip-path:inset(0 100% 0 0)}}
...
<div class="modal" id="modal"><div class="sheet">
  ...
  <div class="mctrl">
    <button id="mreveal" class="reveal" aria-expanded="false">เฉลย</button>
    <button id="mpt" title="ตารางธาตุ"><span class="nw">เปิด</span><wbr><span class="nw">ตารางธาตุ</span></button>
    <button id="mshot" title="บันทึกโจทย์ข้อนี้เป็นไฟล์ PNG"><span class="nw">💾 บันทึก</span><wbr><span class="nw">เป็นรูป</span></button>
  </div>
  <div class="mfrow"><span class="mhint" id="mhint"><span class="hc">‹ ปัดซ้าย/ขวาเพื่อเปลี่ยนข้อ ›</span><span class="hf">← → เปลี่ยนข้อ</span></span><span class="mpos" id="mpos"></span></div>
...
function footHtml(q){return `📄 ${q.source}`;}   // the official key lives behind the เฉลย button now
...
/* ONE reveal per question: answer line (official key, else the worked answer), then the worked solution, then the 📌 note;
   a missing section is left out and the label names what's inside. ⚠ = pending / flagged working. */
const ansTxt=q=>keyTxt(q)?`เฉลยทางการ: ${keyTxt(q)}`:q.solAnswer?`ตอบ: ${q.solAnswer}`:"";
const hasRev=q=>!!(ansTxt(q)||q.solHtml||q.note);
function revName(q){const a=!!ansTxt(q),w=!!q.solHtml;
  return (q.solPending||q.solFlag?"⚠ ":"")+(a&&w?"เฉลย + วิธีทำ":a||isEx(q)&&w?"เฉลย":w?"วิธีทำ":q.note?"หมายเหตุ":"เฉลย");}
function solLabel(q){return revName(q)+(q.solPending?" (ยังไม่ได้ตรวจ)":q.solFlag?" (ไม่ฟันธง)":"")+" →";}
function revBox(q){const a=ansTxt(q);
  return `<div class="solbox">${q.solPending?`<div class="soldis">⚠ เฉลยข้อนี้ยังไม่ได้ตรวจยืนยัน — อาจมีจุดผิด คิดตามทีละขั้นแล้วตัดสินเอง</div>`
...
function solHtml(q){
  ...
  return `<button class="solbtn${q.solFlag||q.solPending?" flag":""}" data-sol="1" data-label="${label}" data-name="${revName(q)}">${label}</button>${revWrap(q)}`;
}
...
function renderPoster(R){
  ...
  R.innerHTML=`<div class="deckbar"><button class="dbtn" id="pprev" title="ก่อนหน้า (←)">←</button>
    ...
    <span class="deckhint">← → เปลี่ยนข้อ · Enter เปิดเฉลย · ลากแถบเพื่อกระโดด · แตะตัวเลือกเพื่อวงไว้</span></div>
...
/* ---- present modal ---- */
let mIdx=0;
/* background scroll lock, shared by every overlay (present mode + the 4 side drawers). Each holds it under its own key
   (lock twice / unlock twice is harmless); the page is released only when the LAST one closes, so a drawer opened over
   present mode doesn't unlock early. iOS Safari still scrolls a body that only has overflow:hidden, so the body is
   pinned position:fixed at -scrollY and put back on exactly that scrollY on release. */
const SL=(()=>{const held=new Set();let y=0;
  function lock(k){if(held.has(k))return;held.add(k);if(held.size>1)return;
    y=window.scrollY;const sw=window.innerWidth-document.documentElement.clientWidth,b=document.body.style;   // desktop scrollbar width: keep the layout still
    Object.assign(b,{position:"fixed",top:-y+"px",left:"0",right:"0",paddingRight:sw>0?sw+"px":""});document.documentElement.classList.add("scrlock");}
  function unlock(k){if(!held.delete(k)||held.size)return;const b=document.body.style;
    Object.assign(b,{position:"",top:"",left:"",right:"",paddingRight:""});document.documentElement.classList.remove("scrlock");
    window.scrollTo(0,y);}
  return{lock,unlock,held:()=>[...held]};})();
// a finger dragged on a dimmed backdrop itself: nothing there scrolls, so don't let it bounce the pinned page
["#modal","#pdscrim"].forEach(s=>$(s).addEventListener("touchmove",e=>{if(e.target===e.currentTarget&&e.cancelable)e.preventDefault();},{passive:false}));
function openModal(i){mIdx=i;fillModal();$("#modal").classList.add("show");SL.lock("modal");INK.open();SFX.play(isEx(filtered[i])?"exOpen":"present");}
function closeModal(){if(!$("#modal").classList.contains("show"))return;$("#modal").classList.remove("show");SL.unlock("modal");INK.close();SFX.play("dismiss");
...
function fillModal(anim){
  ...
  const sh=$("#modal .sheet");sh.classList.remove("lv-easy","lv-medium","lv-hard");if(q.diff)sh.classList.add("lv-"+q.diff);sh.classList.toggle("ex",isEx(q));sh.classList.remove("inR","inL","outL","outR");sh.style.animation="none";void sh.offsetWidth;if(anim)sh.classList.add(anim);sh.style.animation="";
...
// ← / → and a finger swipe: the card slides out the way it was pushed and the next one comes in from the other side
let mBusy=false;
function step(d){if(mBusy||!filtered.length)return;mBusy=true;SFX.play("flip",d);usedNav();
  $("#modal .sheet").classList.add(d>0?"outL":"outR");
  setTimeout(()=>{mIdx=(mIdx+d+filtered.length)%filtered.length;fillModal(d>0?"inR":"inL");mBusy=false;},170);}
function usedNav(){if(document.body.classList.contains("swiped"))return;ls.set("cqb_swiped","1");$("#mhint").classList.add("gone");
  setTimeout(()=>document.body.classList.add("swiped"),650);}
if(ls.get("cqb_swiped","")==="1")document.body.classList.add("swiped");
/* swipe = one FINGER moving mostly sideways >= 50px on the question card. Pencil (touchType "stylus") writes and never
   navigates; the split board and its divider sit outside .sheet; a wide equation that scrolls sideways keeps its own pan.
   Touch events, not pointer events: the browser's vertical scroll cancels pointers but still delivers touchend. */
(()=>{const sh=$("#modal .sheet");let s0=null,pen=false;
  sh.addEventListener("pointerdown",e=>{pen=e.pointerType==="pen";},true);   // Pencil/stylus where Touch has no touchType (non-Safari)
  sh.addEventListener("touchstart",e=>{s0=null;const t=e.touches[0];if(e.touches.length!==1||pen||t.touchType==="stylus")return;
    for(let el=e.target;el&&el!==sh;el=el.parentElement)if(el.scrollWidth>el.clientWidth+1&&/auto|scroll/.test(getComputedStyle(el).overflowX))return;
    s0={x:t.clientX,y:t.clientY};},{passive:true});
  sh.addEventListener("touchmove",e=>{if(e.touches.length!==1)s0=null;},{passive:true});
  sh.addEventListener("touchcancel",()=>{s0=null;});
  sh.addEventListener("touchend",e=>{if(!s0)return;const t=e.changedTouches[0],dx=t.clientX-s0.x,dy=t.clientY-s0.y;s0=null;
    if(Math.abs(dx)>=50&&Math.abs(dx)>Math.abs(dy)*1.5)step(dx<0?1:-1);},{passive:true});})();
...
document.addEventListener("keydown",e=>{
  ...
  if($("#modal").classList.contains("show")){
    ...
    if(e.key==="Enter"&&!e.target.closest("button")){e.preventDefault();$("#mreveal").click();}
...
const PD=(()=>{
  ...
  function build(){
    ...
    h+=`<div class="pdinfo"><div class="pdbig" id="pdbig" tabindex="0" title="แตะเพื่อพลิกดูการจัดเรียงอิเล็กตรอน"><div class="bf"></div></div><div class="pdbrk" id="pdbrk"></div></div><div class="pdcfg" id="pdcfg"></div>`;
    G.innerHTML=h;
  ...
  function cfg(z){let r=z;const occ=SUB.map(([,l])=>{const k=Math.min(r,4*l+2);r-=k;return k;});
    ...
    const t=SUB.map((s,i)=>[s[0]+"spdf"[s[1]],occ[i],s[0]]).filter(x=>x[1]),g=[];t.forEach(x=>(g[x[2]-1]=g[x[2]-1]||[]).push(x));   // g = subshells grouped by shell n
    ...
    return{t,g:g.filter(Boolean),note};}
  ...
  let shown=0;
  const big=(e,fx)=>{const b=$("#pdbig"),cf=$("#pdcfg"),z=e?e.z:0;
    if(z!==shown&&G.classList.contains("bk")){G.classList.add("bstill");G.classList.remove("bk");void cf.offsetWidth;G.classList.remove("bstill");}
    b.classList.toggle("nil",!e);shown=z;
    if(e){last=e;b.firstChild.innerHTML=`<i>${e.z}</i>${e.s}<u>${e.n}</u><s>${fm(e)}</s>`;const c=cfg(e.z),su=([s,k])=>`<span>${s}<sup>${k}</sup></span>`;
      cf.innerHTML=`<b>${e.s}<i>Z ${e.z}</i></b><div class="cg">${c.g.map(g=>`<span class="g"><span>${g.map(su).join(" ")}</span><i></i><strong>${g.reduce((a,x)=>a+x[1],0)}</strong></span>`).join("")}</div>`
        +`<p><em>เขียนตามลำดับการบรรจุ:</em>${c.t.map(su).join(" ")}</p>${c.note?`<small>${c.note.replace("★","<i>★</i>")}</small>`:""}`;}
    else{b.firstChild.innerHTML=`<i>&nbsp;</i>?<u>แตะธาตุเพื่อเปิด</u><s>&nbsp;</s>`;cf.innerHTML="";}
    if(fx){b.classList.remove("pdrev");void b.offsetWidth;b.classList.add("pdrev");}};
  function flipBig(){if(!shown)return;G.classList.toggle("bk");SFX.play("bigFlip",G.classList.contains("bk"));}
  ...
  function open(){SL.lock("pd");if(TD.isOpen())TD.close();if(!built)build();else if(memOn)setMem(false,1);refreshQ();document.body.classList.add("pdopen");SFX.play("drawer","pd");}
  function close(){if(!isOpen())return;document.body.classList.remove("pdopen");SL.unlock("pd");SFX.play("undrawer");}
  ...
  G.addEventListener("click",e=>{if(e.target.closest(".pdbig,.pdcfg"))return flipBig();const t=e.target.closest(".pde");if(!t)return;const el=BY[t.dataset.z];
...
async function saveShot(){const q=filtered[mIdx],b=$("#mshot");if(!q||b.disabled)return;
  const lbl='<span class="nw">💾 บันทึก</span><wbr><span class="nw">เป็นรูป</span>',k=shotKey();const nm=b=>`${q.id}.${b.type==="image/jpeg"?"jpg":"png"}`;
  if(shot.key===k&&shot.blob){deliverShot(shot.blob,nm(shot.blob));SFX.play("saved");b.textContent="✅ บันทึกแล้ว";setTimeout(()=>{b.innerHTML=lbl;},1200);return;}
  ...
  try{if(!(shot.key===k&&shot.job)){const job=renderShot();shot={key:k,blob:null,job};}
    ...
    SFX.play("saved");b.textContent="✅ บันทึกแล้ว";setTimeout(()=>{b.innerHTML=lbl;},1200);
  }catch(e){shot={key:"",blob:null,job:null};b.innerHTML=lbl;console.error(e);alert(e&&e.cdn?"บันทึกรูปไม่ได้ — ต้องต่ออินเทอร์เน็ตครั้งแรกเพื่อโหลดตัวสร้างรูป":"บันทึกรูปไม่ได้ — สร้างรูปข้อนี้ไม่สำเร็จ ลองอีกครั้ง");}
...
const TD=(()=>{
  ...
  function open(){SL.lock("td");if(PD.isOpen())PD.close();if(!built)build();
    const was=fxFor;fxRender();if(was!==fxFor)$("#td").scrollTop=0;
    document.body.classList.add("tdopen");fxFit();SFX.play("drawer","td");}
  function close(){if(!isOpen())return;document.body.classList.remove("tdopen");SL.unlock("td");SFX.play("undrawer");}
...
const VS=(()=>{
  ...
  function open(){SL.lock("vs");if(PD.isOpen())PD.close();if(TD.isOpen())TD.close();
    const f=$("#vsf");if(!f.src)f.src="vsepr.html?embed=1";
    document.body.classList.add("vsopen");SFX.play("drawer","vs");}
  function close(){if(!isOpen())return;document.body.classList.remove("vsopen");SL.unlock("vs");SFX.play("undrawer");}
...
const TM=(()=>{
  ...
  function open(){SL.lock("tm");document.body.classList.add("tmopen");SFX.play("drawer","tm");}
  function close(){if(!isOpen())return;document.body.classList.remove("tmopen");SL.unlock("tm");SFX.play("undrawer");}
...
// exam papers only: exercises have their own ✎ button + ▾ bars on the tile (the old 5th "✎ แบบฝึกหัด" chip is gone, 2026-09-26)
...
ls.set("cqb_papers",JSON.stringify(PSEL));   // write the cleaned list back: drops a stale "ex" saved by the old chip
const EXC={};DATA.questions.forEach(q=>{if(isEx(q))EXC[q.ch]=(EXC[q.ch]||0)+1;});   // exercises per chapter
const examSel=()=>PSEL.length===PAPERS.length?(EXN?EXONLY:""):PSEL.join("+");   // every paper on = every exam, still no exercises
const RM=matchMedia("(prefers-reduced-motion:reduce)");
function statBody(ch){
  const px=PSEL,qs=DATA.questions.filter(q=>subjOf(q)==="chem"&&q.ch===ch&&px.includes(q.exam)),cnt={},df={easy:0,medium:0,hard:0};
  ...
  return `<div class="sth"><span class="sn">${n}</span><span class="snm">${DATA.chapters[ch]}<small>${qs.length} ข้อ${px.length<PAPERS.length?" · "+px.map(k=>PAPERS.find(p=>p[0]===k)[1]).join(" + "):""}</small></span>
  ...
  <div class="sbody"><div class="pchips" role="group" aria-label="สนามสอบ">${PAPERS.map(([k,l])=>{const on=PSEL.includes(k);
    ...
    ${EXC[ch]?`<div class="exi"><div class="exhd"><button class="sexb" data-go="exch" data-ch="${ch}">✎ +${EXC[ch]} แบบฝึกหัด</button>
      <button class="xtg" data-x="st${ch}" aria-expanded="${XOPEN.has("st"+ch)}" aria-controls="xb-st${ch}" aria-label="หัวข้อย่อยแบบฝึกหัดบท ${n}" title="หัวข้อย่อยแบบฝึกหัด"><i aria-hidden="true">▾</i></button></div>
      ${xBars(ch,"st"+ch)}</div>`:""}</div>`;}
...
function exBody(){
  const rows=Object.keys(EXC).sort().map((ch,k)=>{const qs=DATA.questions.filter(q=>isEx(q)&&q.ch===ch),df={easy:0,medium:0,hard:0};
    ...
    return `<div class="exi"><div class="exhd"><button class="exr" data-go="exch" data-ch="${ch}" data-k="${k}" title="เปิดแบบฝึกหัดบท ${parseInt(ch)}"><span class="exn"><small>บท</small>${parseInt(ch)}</span>
      ...
      <span class="exbar">${["easy","medium","hard"].filter(d=>df[d]).map(d=>`<i class="sd-${d}" style="flex:${df[d]}" title="${DIFF_TH[d]} ${df[d]} ข้อ"></i>`).join("")}</span></button>
      <button class="xtg" data-x="ex${ch}" aria-expanded="${XOPEN.has("ex"+ch)}" aria-controls="xb-ex${ch}" aria-label="หัวข้อย่อยแบบฝึกหัดบท ${parseInt(ch)}" title="หัวข้อย่อย"><i aria-hidden="true">▾</i></button></div>
      ${xBars(ch,"ex"+ch)}</div>`;}).join("");
  ...
  <div class="sbody"><div class="scap">แยกตามบท · แตะเพื่อดูข้อ · ▾ หัวข้อย่อย</div><div class="exrows">${rows}</div>
    <div class="sdw"><div class="sleg">${["easy","medium","hard"].map(d=>`<span class="diff-txt de-${d}">${DIFF_TH[d]}</span>`).join("")}</div></div></div>`;}
// ✎ sub-topic bars: one per sub-topic with ≥1 exercise in ch (most first), fill = its share of the chapter's exercises.
// key = the toggle that owns the list; XOPEN = which lists are dropped open (kept across a re-render, cleared when a tile opens)
const XOPEN=new Set();
function xBars(ch,key){
  const qs=DATA.questions.filter(q=>isEx(q)&&q.ch===ch),cnt={};qs.forEach(q=>q.topics.forEach(t=>cnt[t]=(cnt[t]||0)+1));
  const bars=(ST[ch]||[]).filter(t=>cnt[t.id]).sort((a,b)=>cnt[b.id]-cnt[a.id]).map((t,k)=>{const l=`<span class="xl"><span>✎ ${t.th}</span><b>${cnt[t.id]}</b></span>`;
    return `<button class="xbar" data-go="extopic" data-ch="${ch}" data-t="${t.id}" data-k="${k}" style="--w:${cnt[t.id]/qs.length*100}%;--k:${k}" title="${t.th} · ${cnt[t.id]} ข้อ">${l}<span class="xf" aria-hidden="true">${l}</span></button>`;});
  return `<div class="xbars" id="xb-${key}" role="group" aria-label="แบบฝึกหัดบท ${parseInt(ch)} แยกหัวข้อย่อย"${XOPEN.has(key)?"":" hidden"}>${bars.join("")}</div>`;}
// drop a list open / fold it back, then re-pack the tile to its new height (tiles that move slide there)
function xToggle(b){
  const k=b.dataset.x,on=!XOPEN.has(k),B=$("#board"),kids=[...B.children].filter(el=>!el.classList.contains("deco")),F=new Map(kids.map(el=>[el,el.getBoundingClientRect()]));
  on?XOPEN.add(k):XOPEN.delete(k);b.setAttribute("aria-expanded",on);document.getElementById("xb-"+k).hidden=!on;SFX.play("chip",on);
  repack();B.querySelectorAll(".tile.deco").forEach(d=>d.style.animationDelay=".12s");
  if(!RM.matches)kids.forEach(el=>{const a=F.get(el),r=el.getBoundingClientRect();
    if(Math.abs(a.left-r.left)+Math.abs(a.top-r.top)>.5)el.animate([{transform:`translate(${a.left-r.left}px,${a.top-r.top}px)`},{transform:"none"}],{duration:440,easing:"cubic-bezier(.2,.9,.3,1)"});});
  if(on)document.getElementById("xb-"+k).scrollIntoView({block:"nearest",behavior:RM.matches?"auto":"smooth"});}
...
function statSet(t,open,tx=0,dir=open?-1:1){
  ...
  const go=()=>{
    ...
    if(open){XOPEN.clear();t.querySelector(".tback").innerHTML=t.dataset.ch==="ex"?exBody():statBody(t.dataset.ch);}
    repack();B.querySelectorAll(".tile.deco").forEach(d=>d.style.animationDelay=".12s");
...
$("#board").addEventListener("click",e=>{
  ...
  const xt=e.target.closest("[data-x]");if(xt){xToggle(xt);return;}
  ...
  const xg=g==="ex"||g==="exch"||g==="extopic";   // ✎ destinations: exercises only
  SFX.play(g==="topic"?"bar":xg?"exOpen":"tile",+t.dataset.k);setTimeout(()=>SFX.play("whoosh"),70);
  enterApp(x,y,()=>{
    resetFilters();
    if(g==="qid"){$("#q").value=t.dataset.q;}
      ...
      if(EXN&&s==="chem"&&g!=="topic"){const ex=xg?"ex":EXONLY;opt($("#fexam"),ex,ex);$("#fexam").value=ex;}   // ✎ tiles: exercises only; the rest: exams only
      if(g==="ch"||g==="exch"||g==="extopic")$("#fch").value=t.dataset.ch;
      if(g==="extopic"){syncFacets(fstate());$("#ftopic").value=t.dataset.t;}   // a ✎ bar: + that sub-topic
