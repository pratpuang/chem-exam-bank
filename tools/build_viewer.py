...
const SFX=(()=>{let ctx=null,master=null,verb=null,on=ls.get("cqb_sfx","1")!=="0",vol=Math.max(0,Math.min(100,parseInt(ls.get("cqb_vol","70"),10)||0));
  ...
  const P={
    ...
    mark:()=>{noise(.028,{freq:vary(4600,.1),q:3,vol:.09});tone(vary(2300),.022,{vol:.035,type:"triangle",attack:.001});noise(.07,{freq:6500,q:.6,vol:.025,delay:.012});},
    unmark:()=>tone(vary(900),.06,{vol:.05,type:"triangle",slide:.75,attack:.001}),
...
function markChoice(e){const li=e.target.closest(".body li");if(!li)return false;
  ...
  if(on)li.classList.add("mk");SFX.play(on?"mark":"unmark");return true;}
...
function buildBoard(){
  ...
  h+=`<div class="tile c-card w2 recent" style="--i:${i++}"><span class="no">🕘</span><span class="nm">เพิ่งดู</span>${rq.length
    ?`<div class="rl">${rq.map(q=>`<button data-go="qid" data-q="${q.id}" title="${(q.snippet||"").replace(/<[^>]+>/g,"").replace(/"/g,"&quot;").slice(0,90)}">${q.id.slice(2)} · ${q.subject==="bio"?"ชีว":q.subject==="applied"?"ประยุกต์":"บท "+parseInt(q.ch)}</button>`).join("")}</div>`
